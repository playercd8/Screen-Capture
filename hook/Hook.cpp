		...
		if ((lParam & 0xc0000000) == 0xc0000000) {// Key up
			...
			for (int index = 0; index < MAX_KEY; index++) {
				if (hCallWnd[index] == NULL)
					continue;
				if (IsWindow(hCallWnd[index]))
				{
					if ((HotKey[index] == wParam) && (HotKeyMask[index] == MaskBits))
					{
						//SendMessage(hCallWnd[index],WM_HOTKEY,wParam,WM_KEYUP);
						PostMessage(hCallWnd[index], WM_HOTKEY, wParam, WM_KEYUP);
						bProcessed = TRUE;
					}
			...
			for (int index = 0; index < MAX_KEY; index++) {
				...
				if (IsWindow(hCallWnd[index]) && (HotKey[index] == wParam) && (HotKeyMask[index] == MaskBits)) {
					...
					PostMessage(hCallWnd[index], WM_HOTKEY, wParam, WM_KEYDOWN);
					bProcessed = TRUE;
				}
			}
		}
		if (!bProcessed) {
			for (int index = 0; index < MAX_KEY; index++) {
				...
				if (IsWindow(hCallWnd[index]) && (HotKey[index] == 0) && (HotKeyMask[index] == 0))
					...
					PostMessage(hCallWnd[index], WM_HOTKEY, WM_HOTKEY, lParam);
			}
		}
	}
